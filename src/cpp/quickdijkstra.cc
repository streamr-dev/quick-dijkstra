	...
	Napi::Function func =
		DefineClass(env,
				"QuickDijkstra",
					...
					InstanceMethod("computeDistancesFromNode", &QuickDijkstra::computeDistancesFromNode),
...
Napi::Value QuickDijkstra::computeDistancesFromNode(const Napi::CallbackInfo& info)
	{
	int length = info.Length();

	if (length <1 || !info[0].IsNumber())
		{
		Napi::TypeError::New(info.Env(), "1 number expected").ThrowAsJavaScriptException();
		}

	Napi::Number value0 = info[0].As<Napi::Number>();
 	int i = value0.Uint32Value();

	engine.computeDistancesFromNode(i);	
	return Napi::Number::New(info.Env(), 0);
	}
